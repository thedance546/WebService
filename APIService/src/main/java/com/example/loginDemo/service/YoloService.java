...
//    private final String Receipt_URL = "http://receipt-container-:5001/ocr-detection";
    ...
    public ReceiptResponse processReceiptImage(MultipartFile imageFile) throws IOException {
        ...
        // 소, 닭, 돼지를 소고기, 닭고기, 돼지고기로 변경
        Map<String, String> replacementMap = Map.of(
                "소", "소고기",
                "닭", "닭고기",
                "돼지", "돼지고기"
        );
        Set<String> updatedMatchedItemsSet = new HashSet<>();
        for (String item : matchedItemsSet) {
            updatedMatchedItemsSet.add(replacementMap.getOrDefault(item, item));
        }

        // 매칭된 아이템이 없으면 "No matched items"로 처리
        List<String> matchedItems = new ArrayList<>(updatedMatchedItemsSet);
    ...
    private Set<String> matchItems(List<String> items) {
        ...
        List<String> ITEMS_TO_CHECK = Arrays.asList(
                ...
                "쌀", "생 돼지고기", "비엔나 소시지", "사과", "게 맛살", "어묵", "소", "닭", "돼지"
        );
